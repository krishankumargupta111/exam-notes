...
function History() {
  ...
  return (
    <div className='min-h-screen bg-gradient-to-br from-gray-100
    to-gray-200 px-6 py-8'>
      <motion.header
...
shadow-[0_20px_45px_rgba(0,0,0,0.6)]'
      >
        ...
        <div className="flex items-center gap-4 flex-wrap">
          {!isSidebarOpen && <button
            onClick={() => setIsSidebarOpen(true)} className='lg:hidden text-white txt-2xl'>
            <GiHamburgerMenu />
          </button>}
          <button
    ...
    text-white text-sm"
      ...
      p-5 overflow-auto  '>
          ...
          text-gray-200 bg-white/10 hover:bg-white/20 text-start'>
                  ➕ New Notes
                </button>
                <hr className='border-white/10 mb-4'></hr>
                <h2 className='mb-4 text-lg font-bold bg-gradient-to-r
          from-white via-gray-300 to-white bg-clip-text
          text-transparent'>📔 Your Notes</h2>
                {topics.length == 0 && (
...
${activeNoteId === t._id
                      ...
                      <div className='flex flex-wrap gap-2 mt-2 text-xs'>
                        {t.classLevel && <span className='px-2 py-0.5 rounded-full
        bg-indigo-500/20 text-indigo-300'>classLevel : {t.classLevel}</span>}
                        {t.examType && <span className='px-2 py-0.5 rounded-full
        bg-purple-500/20 text-purple-300'>{t.examType}</span>}
                      </div>
        ...
        p-6,min-h-[75vh]"
          ...
          {!loading && !selectedNote && (
            <div className='h-full flex items-center justify-center
              text-gray-400  '>Select a topic from Sidbar</div>
          )}
