...
function Pricing() {
  ...
  return (
    <div className='min-h-screen bg-gray-100 px-6 py-10 relative'>
      <button onClick={() => navigate("/")} className='flex item-center gap-2 text-gray-600
      hover:text-black mb-6'>🔙 Back</button>
...
function PricingCard({
  ...
  return (
    ...
    relative cursor-pointer rounded-xl p-6 bg-white 
    border transition
${isSelected
          ? "border-black"
          : popular
    ...
    `}>
      {popular && !isSelected && <span className='absoltue top-4 right-4 text-xs px-2 py-1 rounded
      bg-indigo-600 text-white'>Popular</span>}
      {isSelected && <span className='absolute top-4 right-4 text-xs px-2 py-1 rounded
      bg-black text-white'>Selected</span>}
...
${isPayingThisCard
  ...
  `}>
      ...
      <ul className='mt-5 space-2 text-sm text-gray-600'>
        {features.map((f, i) => (
