...
const markDownComponent = {
  h1: ({ children }) => (
    <h1 className='text-2xl font-bold text-indigo-700 mt-6
      mb-4 border-b pb-2'>{children}</h1>
  ),

  h2: ({ children }) => (
    <h2 className='text-2xl font-semibold text-indigo-700 mt-6
      mb-3'>{children}</h2>
  ),
  h3: ({ children }) => (
...
function FinalResult({ result }) {
  ...
  return (
    <div className='mt-6 p-3 space-y-10 bg-white'>
      ...
      md:justify-between gap-4 '>
        <h2 className='text-3xl font-bold bg-gradient-to-br from-indigo-600
        to-purple-600 bg-clip-text text-transparent'>
          📔 Generated Notes
        </h2>
        <div className=' flex-gap-3'>
          <button onClick={() => setQuickRevision(!quickRevision)} className={`px-4 py-2 rounded-lg text-sm
  ...
  bg-indigo-600 text-white hover:bg-indigo-700'>
   ...
   '>
      ...
      {quickRevision && <section className='
      rounded-xl bg-gradient-to-r from-green-100
      to-green-50 border border-green-200 p-6'>
        <h3 className='font-bold txt-green-700 mb-3 text-lg'>
      ...
      {result.diagram?.data && <section>
        ...
        <p className='mt-3 text-xs text-gray-500 italic
          '>⬇ If you need this diagram for future reference or
          revision,you can see it by taking a screnshot</p>
      ...
      {result.charts?.length > 0 &&
        <section>
          ...
          <p className='mt-3 text-xs text-gray-500 italic
          '>⬇ If you need this chart for future reference or
            revision,you can see it by taking a screnshot</p>
        </section>}
...
function SectionHeader({ icon, title, color }) {
  ...
  return (
    ...
    mb-4 px-4 py-4
    rounded-lg
    bg-gradient-to-r ${colors[color]} font-semibold flex items-center
    gap-2`}>
      <span>{icon}</span>
      <span>{title}</span>
