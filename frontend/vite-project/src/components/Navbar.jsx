...
import { AnimatePresence, hover, isPrimaryPointer, motion } from 'motion/react'
...
function Navbar() {
  ...
  rounded=2xl bg-gradient-to-br from-black/90 via-black/80
  to-black/90
  backdrop-blur-2xl
  border border-white/10
  shadow-[0_22px_55px_rgba(0,0,0,0,0.75)]
  flex items-center justify-between px-8 py-4'>
      <div className='flex items-center gap-3 '>
        <img src={logo} alt="examnotes" className='w-9 h-9' />
        <span className='text-lg hidden md:block font-semibold
      text-white'>ExamNotes<span className='text-gray-400'>
            AI</span>
        </span>
      ...
      <div className='flex items-center gap-6 relative'>
        ...
        px-2 py-2 rounded-full bg-white/10
        border border-white/20
        text-white text-smshadow-md cursor-pointer
        '>
            <span className='text-xl'>💎</span>
            <span>{credits}</span>
            <motion.span whileHover={{ scale: 1.07 }}
              whileTap={{ scale: 0.97 }} className='ml-2 h-5 w-5
        flex items-center justify-center rounded-full bg-white
         text-black text-xs font-bold'>➕</motion.span>
          </motion.div>
          <AnimatePresence>
            {showCredits &&
  ...
  p-4 text-white'>
                ...
                <MenuItem text="Sign out" red onClick={handlSignOut} />
...
function MenuItem({ onClick, text, red }) {
  return (
    ...
    w-full text-left px-5 py-3 text-
    sm
